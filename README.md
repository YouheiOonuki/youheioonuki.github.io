...
2. `index.html` の、合うカテゴリの `<ul class="tools">` にある `<li>` を複製し、リンク先・アイコン・説明（短い 1 文）を書き換える。**トップに載せるのはリポジトリの入口（`./<リポジトリ名>/`）だけ**で、子ページはそのツールの入口ページからリンクする（yorozu-plans ROADMAP 7.9.2）。カテゴリは「くらし・お金」／「集まり・イベント」／「情シス・IT」／「変換・データ」／「まなび・あそび」／「親・家族のために（広告なし）」の 6 つ（2026-09-25。親・家族の節だけは、別リポジトリの広告なしページへの行も置く）。「新しいツール」は 3 件で、公開後の再点検（iPhone・Android で 1 回ずつ操作、印刷物は 1 枚印刷）を済ませたものだけを載せ、月 1 回入れ替える（REVIEW C7 R11）
...
25. **画面の骨組み**（yorozu-plans の `docs/SCREEN.md`。2026-09-24 決定 D59〜D66）: 入力 → 結果（計算機・道具）か、設定 → 出力（印刷物・ダウンロード型）のどちらかにする（SCREEN.md 1 章）
   ...
   - 結果（印刷物は印刷ボタン）が画面の外にあるときだけ、**上端**に 44px の固定バー（数字 1 つと「結果へ」）を出す。下端は AdSense のアンカー広告が使う。PC（900px 以上）の 2 カラムは制度の計算機だけ
   - 印刷では固定バーと広告を消す
